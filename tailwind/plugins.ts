...
const extractCSSColors = ({ colorNames }: { colorNames: (keyof typeof colors)[] }) => {
    try {
      const colorsToExport = colorNames.map((name) => ({ name, colorObj: colors[name] })),
        colorVars = colorsToExport.reduce(
          (acc, { name, colorObj }) => ({ ...acc, ...makeColorNames(name, colorObj) }),
          {}
        )
      return colorVars
    } catch (err) {
  ...
  makeColorNames = (
    ...
    return Object.entries(colorObj).reduce((acc, [weight, value]) => {
      if (weight === 'DEFAULT') return { ...acc, [`--color-${colorName}`]: value }
  ...
  extractBackgorundImages = () => {
    try {
      return Object.entries(bgs)
        ...
        .reduce((acc, { name, bg }) => ({ ...acc, ...makeBgName(name, bg) }), {})
    } catch (err) {
      console.log('Failed to extract colors from theme', err)
    }
  },
  ...
  getRootCSSVariables = () => {
    const color =
        extractCSSColors({
          colorNames: ['muro', 'cherry', 'grape', 'neutral', 'white', 'alert'],
        }) ?? {},
